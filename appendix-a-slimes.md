...
| Shadow | None | N/A | Grey Labyrinth (dark corners, Shadow Pots) | No  | Cannot be vacuumed up. Cannot be eaten by Tarr. Spawns in groups of 2-5. Produces plorts when hit by any thrown object (item is not consumed and can be reused). Sits motionless when undisturbed, jumps erratically when disturbed, then vanishes after a few seconds. Will despawn if left alone for several minutes. **Patch 1.1.0:** Shadow Pots now spawn throughout the Grey Labyrinth - break them open to release Shadow Slimes. Shadow Pots can be picked up and moved to different locations for strategic plort farming. **Patch 1.1.1:** Shadow Slimes now despawn once they've produced their maximum plort count, capping the yield from any single spawn. Shadow Pots now contain fewer Shadow Slimes per pot - work multiple pots in sequence rather than camping one. |
